...
function loadEventListeners() {
    ...
    document.addEventListener('keyup', enterInputs);
    // Clear all inputs
    ...
    enterNote.addEventListener('click', enterNoteClicked)
};

function addInputs(name, team, time) {
    let text = `<li class="collection-item blue lighten-5"><span class="capitalize">${name}</span> <span class="capitalize">(${team})</span> joined the call at ${time}.</li>`
...
function enterInputs(event) {
    if (event.keyCode == 13) {
        if (nameInput && nameInput.value && teamInput && teamInput.value) {
            ...
            let options = { hour: 'numeric', minute: 'numeric', second: 'numeric', month: 'numeric', day: 'numeric', year: '2-digit' };
...
function addNotes(time, notes) {
    let text = `<li class="collection-item blue lighten-5">${time} - <span class="">${notes}</span></li>`
    let position = "beforeend";
    conversationOutput.insertAdjacentHTML(position, text);
}

function enterNoteClicked() {
